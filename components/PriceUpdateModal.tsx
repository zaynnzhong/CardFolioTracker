...
interface PriceUpdateModalProps {
  card: Card;
  onSave: (cardId: string, newPrice: number, date: string, platform?: string, variation?: string, grade?: string, serialNumber?: string) => void;
  onCancel: () => void;
}
...
export const PriceUpdateModal: React.FC<PriceUpdateModalProps> = ({ card, onSave, onCancel }) => {
  ...
  const [variation, setVariation] = useState<string>('Base');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isNaN(price)) {
      console.log('[PriceUpdateModal] Submitting:', { cardId: card.id, price, priceDate, platform, variation, grade, serialNumber });
      onSave(card.id, price, priceDate, platform, variation, grade, serialNumber || undefined);
    }
  };
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Silver, Red, Base"
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2.5 text-sm text-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
                />
              </div>
